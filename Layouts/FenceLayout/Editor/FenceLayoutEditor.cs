...
namespace Castrimaris.FenceLayout.Editor {
    /// <summary>
    /// A <see cref="Component"/> copied from somewhere that is useful for fence layouts and placing. This is the Editor part.
    /// </summary>
    [CustomEditor(typeof(FenceLayout)), CanEditMultipleObjects]
    public class FenceLayoutEditor : UnityEditor.Editor {
        ...
        public override void OnInspectorGUI() {
            ...
            if (numPrefabs != m_fenceLayout.FencePrefabs.Length) {
                Array.Resize(ref m_fenceLayout.FencePrefabs, numPrefabs);
            }

            for (int i = 0; i < m_fenceLayout.FencePrefabs.Length; i++) {
                m_fenceLayout.FencePrefabs[i] = EditorGUILayout.ObjectField("Fence prefab " + i,
                    m_fenceLayout.FencePrefabs[i], typeof(Transform), true) as Transform;
            }

            if (!m_fenceLayout.ObjectMode) {
                m_fenceLayout.PostPrefab =
                    EditorGUILayout.ObjectField("End post prefab", m_fenceLayout.PostPrefab, typeof(Transform), true) as
                        Transform;
            }
            ...
            if (m_fenceLayout.ObjectMode) {
                m_fenceLayout.PositionVariation =
                    EditorGUILayout.Vector2Field("Position variation", m_fenceLayout.PositionVariation);
                m_fenceLayout.RotationVariation =
                    EditorGUILayout.Vector2Field("Rotation variation", m_fenceLayout.RotationVariation);
                m_fenceLayout.ScaleVariation =
                    EditorGUILayout.Vector2Field("Scale variation", m_fenceLayout.ScaleVariation);
            } else {
            ...
            if (m_fenceLayout.UseRaycast) {
                m_fenceLayout.RaycastOffsetMax =
                    EditorGUILayout.FloatField("Raycast offset max", m_fenceLayout.RaycastOffsetMax);
                m_fenceLayout.RaycastOffsetMin =
                    EditorGUILayout.FloatField("Raycast offset min", m_fenceLayout.RaycastOffsetMin);
            ...
            if (ShowPoints) {
                ...
                if (GUILayout.Button("Add point")) {
                    ...
                    if (m_fenceLayout.FencePoints.Count > 1) {
                        Vector3 dir = m_fenceLayout.FencePoints[m_fenceLayout.FencePoints.Count - 1] -
                                      m_fenceLayout.FencePoints[m_fenceLayout.FencePoints.Count - 2];
                    ...
                    if (fence) {
                        ...
                        if (e.control) {
                            ...
                            if (Physics.Raycast(r, out result)) {
                                m_editMode = (e.control && e.shift)
                                    ? FenceLayout.EditMode.Delete
                                    : (e.control ? FenceLayout.EditMode.Add : FenceLayout.EditMode.None);
        ...
        public void CreateFences() {
            ...
            if (m_fenceLayout.FencePrefabs.Length > 0 && m_fenceLayout.FencePrefabs[0] != null) {
                ...
                for (int p = 0; p < numLines; p++) {
                    Vector3 point = m_fenceLayout.FencePoints[p % m_fenceLayout.FencePoints.Count] +
                                    m_fenceLayout.transform.position;
                    ...
                    if (gotLast) {
                        ...
                        Vector3 endOffset = Vector3.Scale(m_fenceLayout.FenceEndOffset, m_fenceLayout.FenceScale) *
                                            scale;
                        ...
                        if (m_fenceLayout.ObjectMode) {
                            ...
                            for (int i = 0; i < fenceCount; i++) {
                                ...
                                Vector3 offset =
                                    new Vector3(
                                        Random.Range(-m_fenceLayout.PositionVariation.x,
                                            m_fenceLayout.PositionVariation.x), 0.0f,
                                        Random.Range(-m_fenceLayout.PositionVariation.y,
                                            m_fenceLayout.PositionVariation.y));
                                offset = offsetRot * offset;

                                float rotVariance = Random.Range(m_fenceLayout.RotationVariation.x,
                                    m_fenceLayout.RotationVariation.y);

                                rot = Quaternion.Euler(0, m_fenceLayout.FenceRotation + rotDeg + rotVariance, 0);
                                float scaleVariance = Random.Range(m_fenceLayout.ScaleVariation.x,
                                    m_fenceLayout.ScaleVariation.y);
                                ...
                                createPos.y = y1;
                                Transform fence =
                                    PrefabUtility.InstantiatePrefab(GetRandomFencePrefab(), m_fenceLayout.transform) as
                                        Transform;
                                if (fence) {
                            ...
                            for (int i = 0; i < numFencesInt; i++) {
                                ...
                                if (m_fenceLayout.UseShear) {
                                    ...
                                    if (Mathf.Abs(shearAngleRad) > 0.01f) {
                                        ...
                                        t1.transform.localScale = new Vector3(1.0f, Mathf.Cos(shearAngleRad * 0.5f),
                                            Mathf.Sin(shearAngleRad * 0.5f));
                                        t2.transform.localRotation = Quaternion.Euler(-45.0f, 0, 0);
                                        t2.transform.localScale = new Vector3(1.0f, Mathf.Sqrt(2.0f),
                                            Mathf.Sqrt(2.0f) / Mathf.Sin(shearAngleRad));
                                        Transform fence =
                                            PrefabUtility.InstantiatePrefab(GetRandomFencePrefab(), t2.transform) as
                                                Transform;
                                        if (fence) {
                                            fence.localScale = new Vector3(m_fenceLayout.FenceScale.x,
                                                m_fenceLayout.FenceScale.y, m_fenceLayout.FenceScale.z * scale);
                                            fence.localRotation =
                                                Quaternion.Euler(0.0f, m_fenceLayout.FenceRotation, 0.0f);
                                    ...
                                    Transform fence =
                                        PrefabUtility.InstantiatePrefab(GetRandomFencePrefab()) as Transform;
                                    if (fence) {
            ...
            if (!m_fenceLayout.CompleteLoop && !m_fenceLayout.ObjectMode && m_fenceLayout.PostPrefab) {
                if (m_fenceLayout.FencePoints.Count > 0) {
                    Transform post =
                        PrefabUtility.InstantiatePrefab(m_fenceLayout.PostPrefab, m_fenceLayout.transform) as Transform;
                    if (post) {
        ...
        private Transform GetRandomFencePrefab() {
            ...
            return m_fenceLayout.FencePrefabs[idx] != null
                ? m_fenceLayout.FencePrefabs[idx]
                : m_fenceLayout.FencePrefabs[0];
        }
